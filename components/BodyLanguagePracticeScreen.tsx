...
import { bodyLanguagePrewrittenGameData } from '../data/bodyLanguagePracticeData';

type GameState = 'start' | 'playing' | 'feedback' | 'end';

const BodyLanguagePracticeScreen: React.FC = () => {
    ...
    const currentLevel = bodyLanguagePrewrittenGameData[levelIndex];
    ...
    const handleNextLevel = () => {
        ...
        setHintUsed(false);
        setShowHintText(false);
        if (levelIndex + 1 < bodyLanguagePrewrittenGameData.length) {
    ...
    const highlightedScenario = () => {
        if (!showHintText) return currentLevel.scenario;
        let scenarioHTML = currentLevel.scenario;
        currentLevel.cues.forEach(cue => {
            const regex = new RegExp(`(${cue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
            scenarioHTML = scenarioHTML.replace(regex, `<strong class="bg-yellow-200 px-1 rounded">$1</strong>`);
        });
        return scenarioHTML;
    };
    ...
    if (gameState === 'start') {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-4">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Body Language Challenge</h1>
                <p className="text-gray-600 mb-8 max-w-md">Test your ability to interpret non-verbal communication cues and body language signals.</p>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 flex items-center justify-center">
            <div className="w-full max-w-3xl bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-6 md:p-10">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold">Scenario {levelIndex + 1}</h2>
                    <div className="text-lg font-semibold text-blue-600">Score: {score}</div>
                </div>
                <div className="bg-gray-50/70 p-6 rounded-lg mb-6 border">
                    <h3 className="text-lg font-semibold mb-2">Body Language Scenario</h3>
                    <p dangerouslySetInnerHTML={{ __html: highlightedScenario() }} className="text-gray-700 leading-relaxed"></p>
                </div>
                ...
                {gameState === 'feedback' && (
                    <div className={`p-4 rounded-lg mt-4 animate-fadeIn ${isCorrect ? 'bg-green-100 border-l-4 border-green-500' : 'bg-red-100 border-l-4 border-red-500'}`}>
                        ...
                        {!isCorrect && <p className="font-semibold text-red-700 mt-1">Correct interpretation: {currentLevel.correctAnswer}</p>}
                        ...
                        <button onClick={handleNextLevel} className="w-full bg-blue-600 text-white font-bold py-2 px-10 rounded-lg shadow-md hover:bg-blue-700 mt-4">
                           {levelIndex + 1 < bodyLanguagePrewrittenGameData.length ? 'Continue' : 'Finish'}
                        </button>
                    </div>
                )}
            </div>
...
export default BodyLanguagePracticeScreen;
