...
const Big5RetrievalScreen: React.FC = () => {
    const [selectedTrait, setSelectedTrait] = useState<string>(Object.keys(big5Data)[0]);
    ...
    const handleSubmit = async () => {
        if (!userInput.trim() || !selectedTrait) return;
        setIsLoading(true);
        setError(null);
        setFeedback(null);
        try {
            const traitData = big5Data[selectedTrait];
            const result = await evaluateBig5RetrievalPractice(selectedTrait, traitData, userInput);
            setFeedback(result);
        } catch (err) {
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                <div className="bg-white p-6 rounded-xl shadow-lg">
                    <h1 className="text-3xl font-bold text-gray-900">Retrieval Practice: Big-5</h1>
                    <p className="text-gray-600 mt-2">Strengthen your memory by actively recalling trait characteristics. Select a trait, type everything you can remember about it, and get instant AI-powered feedback.</p>

                    <div className="mt-6">
                        <label htmlFor="trait-select" className="block text-sm font-medium text-gray-700 mb-2">
                            Select Trait
                        </label>
                        <select
                            id="trait-select"
                        ...
                        >
                            {Object.keys(big5Data).map(trait => (
                    ...
                    <div className="mt-4">
                        <label htmlFor="trait-input" className="block text-sm font-medium text-gray-700 mb-2">
                            Recall Trait Characteristics
                        </label>
                        <textarea
                            id="trait-input"
                            rows={8}
                            ...
                            placeholder={`e.g., "High scorers are creative and curious, while low scorers are more conventional..."`}
...
export default Big5RetrievalScreen;